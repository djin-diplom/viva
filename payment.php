<section class="payment">
    <div>Оплатить работу вы можете<br><a href="oplata.php">наличными в офисе компании</a><br>либо с помощью платежных систем:</div>
    <!--
    <div><a href="/oplata/#visa"><img class=desaturated src="/img/pay1.gif" width=78 height=37 alt=""></a></div>
    <div><a href="/oplata/#mastercard"><img class=desaturated src="/img/pay2.gif" width=62 height=37 alt=""></a></div>
    -->
    <div><a href="oplata.php"><img class="desaturated" src="./index_files/pay12.gif" width="110" height="37" alt=""></a></div>
    <div><a href="oplata.php"><img class="desaturated" src="./index_files/pay4.gif" width="122" height="37" alt=""></a></div>
    <div><a href="oplata.php"><img class="desaturated" src="./index_files/pay3.gif" width="85" height="37" alt=""></a></div>
    <div><a href="oplata.php"><img class="desaturated" src="./index_files/pay6.gif" width="135" height="37" alt=""></a></div>
    <div><a href="oplata.php"><img class="desaturated" src="./index_files/pay5.gif" width="76" height="37" alt=""></a></div>
</section>